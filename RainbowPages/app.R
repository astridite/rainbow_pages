...
ui <-  fluidPage(
                        ...
                        sidebarLayout(
                          sidebarPanel(
                            ...
                            checkboxGroupInput('layer', 
                                               ...
                                               width='100%'), 
                            width=2),
                          mainPanel((DT::dataTableOutput(outputId = "browse")))
                          ))),
             # Browse the map of Cape Town: 
  ...
  tags$style(HTML("a {color: #eb34c0}")), # pink hyperlinks
  tags$style(type = 'text/css', '.navbar-default { background-color: rgba(0, 0, 0, 0);
             color: rgba(0, 0, 0, 0);}'), # clear navbar
...
# Table:
  # Making filterable data for table:
  data <- reactive(
    curated_data %>% 
 ...
 # Custom, searchable data table:
  output$browse <- renderDataTable({
    datatable(data(),
              rownames = F,
              colnames=c("Name", "Detail", "Email", "Online Presence", "Location", "Sector", "colours"),
              options=list(
              columnDefs = list(list(visible=FALSE, targets=c(6))), # removing colour index from view
              paging=F)) %>% 
      formatStyle('colours', 
                  target = 'row', 
...
# Map:
  ...
  # Creating markers with icons specific to activity
    filtered_markers <- curated_data %>%
      filter(!is.na(lon), #removing data without locaton
  ...
  observe({
        leafletProxy("map", data = map_filtered()[[1]]) %>%
          clearMarkers() %>% # remove default markers
          clearMarkerClusters() %>% # remove default clusters
          addMarkers( # add new custom markers defined above
            lng = ~lon,
            lat = ~lat,
            icon = map_filtered()[[2]],
            popup = ~as.character(label),
